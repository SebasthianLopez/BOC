...
class FakeWorkplace implements Workplace {
  ...
  async create(title: string, description: string, beforeWrite: () => Promise<void>) { await beforeWrite(); const task = { id: `11111111-1111-4111-8111-${String(++this.creates).padStart(12, "0")}`, title, description, url: this.missingUrl ? null : `https://app.ambiguous.ai/tasks/${this.creates}` }; this.tasks.push(task); return task; }
}
...
test("approval creates every displayed commitment and returns real Ambiguous IDs and URLs", async (t) => {
  ...
  assert.ok(commitments.every((commitment) => commitment.ambiguousId && (commitment.url === null || commitment.url.startsWith("https://app.ambiguous.ai/"))));
});
...
test("create, read-back, refresh, and restart keep real IDs when Ambiguous has no URL", async (t) => {
  const { service, workplace, directory } = await fixture(t);
  workplace.missingUrl = true;
  const prepared = await service.prepare(session, proposal);
  const created = await service.approve(session, prepared.id);
  assert.equal(created[0].url, null);
  assert.ok(created[0].ambiguousId);
  const restarted = new FollowupService(workplace, directory);
  const refreshed = await restarted.list(proposal.decisionId);
  assert.equal(refreshed[0].url, null);
  await restarted.approve(session, prepared.id);
  assert.equal(workplace.creates, 2);
});
